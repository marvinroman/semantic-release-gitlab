          ...
          assets.map(async (asset) => {
            ...
            if (isPlainObject(asset)) {
              if (globbed.length > 1) {
                ...
                const { filepath, fullpath, ...others} = asset;
                // allow user to have full path returned as the label for directories that might have duplication and it's desired to include it
                if (fullpath) {
                  return globbed.map((file) => ({ ...others, path: file, label: file }));
                } else {
